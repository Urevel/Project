...
{
#define A1 ((AAA*)a1) // задаем переменную а1
#define A2 ((AAA*)a2)
...
bool BuildHeap(char* FileName, heap::Heap& h)	// построение кучи из файла, char *FileName - имя файла, heap::Heap& h - куча, на которую берётся указатель
{
	...
	while (inFile >> n) //оператор сдвига (проходимся по файлу)
	{
		int* a = new int;
		*a = n;	// переписываем прочитанный элемент из буфера в выделенную память 
		h.Insert((void*)a); //вставляем элемент в кучу
		cout << *a << endl;
	}
	inFile.close();
	return rc;
...
void SaveHeap(heap::Heap& h, char* FileName) //сохранение (запись) кучи в файл
	{
	...
	{
		a = (int*)h.Storage[u]; //storage - массив элементов произвольного типа
		outFile << *a;
		outFile << endl;
	}
	outFile.close();
